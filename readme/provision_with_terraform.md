# Bonus: Deploying Your Cloud Workload with Terraform and Ansible

In the months since I began running this demonstration, I am regularly asked whether or not an Ansible Tower Workflow can be run while using terraform scripts to provision the cloud infrastructure and instances. The answer is an emphatic yes, and this page includes playbooks and directions for setting up this very scenario. Nearly the same workflow is used to implement self-service cloud deployment, with the only difference being that the actual provisioning and teardown are accomplished with terraform scripts.
